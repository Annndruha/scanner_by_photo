...
blank_img = np.zeros((img_gray.shape[0],img_gray.shape[1]),dtype='uint8')
...
lines = cv2.HoughLines(conturs_map, 2, np.pi/180, 50, None, 0, 0)
#blank_img_rgb = cv2.cvtColor(np.zeros((img_gray.shape[0],img_gray.shape[1]),dtype='uint8'), cv2.COLOR_GRAY2BGR)
